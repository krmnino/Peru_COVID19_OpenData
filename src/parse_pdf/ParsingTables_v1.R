...
# Muertes acumuladas por departamento (SINADEF + MINSA)
...
# Extract table header and convert to list
header <- ConfigLoader_get_value(Main_Config, "MADeptoSM_RTHdr")
header_list <- list()
for(i in 1:length(header)){
  header_list <- append(header_list, header[i])
}
...
# Build raw table path
raw_table_path <- ConfigLoader_get_value(Main_Config, "MADeptoSM_RT")
raw_table_path <- paste(top_level_path, raw_table_path, sep="")
